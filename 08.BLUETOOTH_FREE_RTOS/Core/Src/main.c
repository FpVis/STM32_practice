...
uint8_t rx_data;	//	UART rx_data Byte
uint8_t rx_bt_data;	//	UART BLUETOOTH DATA

volatile int TIM11_1ms_counter = 0;
volatile int TIM11_DHT11_counter = 0;
volatile int TIM11_Ultrasonic = 0;
...
int main(void)
{
  ...
  HAL_TIM_Base_Start_IT(&htim11);				//	Timer Initialize


  HAL_TIM_PWM_Start(&htim5, TIM_CHANNEL_2);		// BUZZER PWM START


  HAL_UART_Receive_IT(&huart2, &rx_data, 1);	//	RECIEVE UART DATA to RX_DATA	--> COMPORT_MASTER
  HAL_UART_Receive_IT(&huart1, &rx_bt_data, 1);	//	RECIEVE UART_BT_DATA to RX_BT_DATA	--> BLUETOOTH


  HAL_TIM_IC_Start_IT(&htim1, TIM_CHANNEL_1);	//	COUNTING PULSE (RISING EDGE TO FALLING EDGE)	--> ULTRASONIC
