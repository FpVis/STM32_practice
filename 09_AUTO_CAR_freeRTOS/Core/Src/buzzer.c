...
extern volatile int TIM11_buzz_counter;
...
void set_buzzer(int frequency)
{
	...
	__HAL_TIM_SET_AUTORELOAD(&htim5, divide_freq / frequency); // PWM
	__HAL_TIM_SET_COMPARE(&htim5, TIM_CHANNEL_2, divide_freq / frequency / 2);  // Duty를 50%로 설정 한다.
}
...
void beep(int repeat, int Hz, int time)
{
	HAL_TIM_PWM_Start(&htim5, TIM_CHANNEL_2);
